...
// NEW: Add file input for syllabus upload
...
let selectedFiles2 = new Set();
let selectedExcelFile = null;
let selectedSyllabusFile = null; // NEW: Track syllabus file
let ctTemplate_File = null;
let tlp_File = null;

// NEW: Function to check if Generate Excel for CT should be enabled
...
// NEW: Handle syllabus file selection (single file only)
function handleSyllabusFileSelection(fileInput, fileNameDisplay) {
    ...
    if (file) {
        ...
        if (isValid) {
            ...
            updateGenerateButton1State(); // Check if button should be enabled
        } else {
...
function handleFileSelection(fileInput, fileNameDisplay, selectedFiles, generateButton) {
    ...
    if (selectedFiles.size > 0) {
        ...
        // MODIFIED: For fileInput1, use the new state update function
...
// NEW: Add event listeners for syllabus upload
...
async function handleCTFileUpload(selectedFiles, url, messageElement, downloadButton) {
    ...
    // NEW: Check if syllabus file is selected
    ...
    // NEW: Append syllabus file to form data
